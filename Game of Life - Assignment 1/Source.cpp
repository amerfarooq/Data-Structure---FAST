...
whether it will propogate to the next generation. If the cell is dead, only counts the neighbours and performs the evaluation. If the cell will live on, returns
...
int main() {
	...
	for (int i = 0; i < totalGenerations; ++i) {
		...
		cellArray = new Cell[totalLiveCells * 9];
		for (int i = 0; i < cellCount; ++i) {
			cout << tempArray[nextGenCellsArray[i]] << " will propogate: " << tempArray[nextGenCellsArray[i]].willPropogate << endl;
			cellArray[i] = tempArray[nextGenCellsArray[i]];
			cellArray[i].isAlive = true;
			cellArray[i].willPropogate = false;
			board[cellArray[i].x][cellArray[i].y] = i;
		}

		for (int i = 0; i < oldCellCount; ++i) {
			// If cell at index i in tempArray is alive and not propogating to next generation, kill it.
			if ( (tempArray[i].isAlive && !tempArray[i].willPropogate) || !tempArray[i].isAlive){
